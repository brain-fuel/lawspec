...
validateUnit u = either (Left . pure . (\m -> Diagnostic "declaration" m Nothing)) Right $ do
  ...
  forM_ (functions u) $ \(n,t) -> do
    ...
    let (args,result) = functionType t
        concrete (Named name) = name `elem` ["Int32","Text","Bool"]
        concrete _ = False
    unless (not (null args) && all concrete (result:args))
      (Left (n ++ ": functions require one or more scalar Int32/Text/Bool inputs and a scalar result"))
  forM_ (laws u) $ \l -> do
...
compile sources = do
  ...
  allExpanded <- forM [(u,l) | u <- us,l <- laws u] $ \(u,l) ->
    either (Left . pure . (\m -> Diagnostic "semantic" m (Just (location l)))) Right $ evalStateT (do
      ...
      (bs,body,tr) <- expand table (unitName u) env [] l (map (Var . fst) (parameters l))
      ...
      pure (Expanded (unitName u) (lawName l) checkedInputs a b gs body tr l)
      ) (CS M.empty 0 [])
  pure (filter ((/= "prelude") . unitName) us, filter (null . parameters . original) allExpanded)

prettyExpanded :: Expanded -> String
prettyExpanded e = "for all " ++ intercalate " " ["(" ++ inputName i ++ " :: " ++ prettyType (inputType i) ++ ")" | i <- inputs e] ++ " . " ++ showAssertion (assertion e)
